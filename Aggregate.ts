    ...
    for (let j in SSID2) {
      for (let n in SSName4) {
        ...
        const Yesterday = moment()
          .add(-1, 'days')
          .format('YYYY/MM/DD');
        await RPA.Logger.info('本日の日付　　　　　→ ' + Yesterday);
        // 前々月の日付をフォーマット変更して取得
        ...
        for (let v in ProgramList) {
          for (let k in ProgramName2) {
            if (ProgramName2[k][0] == ProgramList[v]) {
              ...
              // タイトルリストの末尾を削除
              await TitleList.pop();
              await Work(Today, Yesterday, TwoMonthsBefore);
              break;
            }
          }
        }
...
async function Work(Today, Yesterday, TwoMonthsBefore) {
  try {
    // Youtube Studioにログイン
    ...
    if (ProgramName[0] == 'スポット') {
      ...
      await SetData(Today, Yesterday);
    } else {
      // 番組名を入力
      await SetProgram();
      // 番組名からタイトルを検索
      await GetData(Yesterday, TwoMonthsBefore);
      // シートに取得したデータを記載
      await SetData(Today, Yesterday);
    }
  } catch (error) {
...
async function GetData(Yesterday, TwoMonthsBefore) {
  // タイトルがない場合はスキップ
  try {
...
const Row4 = [];
const Row5 = [];
async function SetData(Today, Yesterday) {
  if (TitleList.length < 1) {
