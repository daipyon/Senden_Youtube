...
// ジャバスククリックもしくはURLで２ページ目以降に飛んでみる
...
async function GetData3(TitleList) {
  ...
  while (LoopFlag2 == '') {
    await RPA.Logger.info('カウント　　　→', Count);
    if (Count % 2 == 0) {
      await RPA.Logger.info('カウント　　　→', Count);
      await RPA.Logger.info('偶数なのでスキップします');
      Count++;
      continue;
    } else {
      // for (let i = 0; i <= 29; i++) {
      ...
      for (let i in TitleList) {
        ...
        for (let n = 0; n <= 29; n++) {
          ...
          // await RPA.Logger.info('N のカウント①　　   →', CountN[0]);
          // if (YoutubeTitleList[n].indexOf(TitleList[i]) == 0) {
          if (TitleList[i].indexOf(Title) == 0) {
            LoopFlag2 == 'true';
            // テスト①
            await RPA.Logger.info('シートのタイトル　　→', TitleList[i]);
            ...
            // テスト①
            // if (CountI[0] == TitleList.length) {
            await RPA.Logger.info('I のカウント②　　　　→', CountI[0]);
            await RPA.Logger.info('N のカウント②　　   →', CountN[0]);

            // ここで取得のループを抜ける
            // if (TitleList[TitleList.length - 1] == YoutubeTitleList[n]) {
            if (TitleList[TitleList.length - 1] == Title) {
              await RPA.Logger.info('I のカウント③　　　　→', CountI[0]);
              await RPA.Logger.info('N のカウント③　　   →', CountN[0]);
            ...
            if (CountI[0] == TitleList.length && CountN[0] == 29) {
              // await RPA.WebBrowser.takeScreenshot();
              LoopFlag2 == 'true';
              await RPA.Logger.info('I のカウント④　　　　→', CountI[0]);
              await RPA.Logger.info('N のカウント④　　   →', CountN[0]);
