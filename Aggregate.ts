            ...
            if (i == 3) {
              ...
              if (TitleList.length != MatchTitleList.length) {
                for (let n in TitleList) {
                  if (MatchTitleList.includes(TitleList[n]) == false) {
                    await RPA.Logger.info(
                      `タイトルリストの ${n} 番目のタイトルを削除します`
                    );
                    await TitleList.splice(n, 1);
                  }
                }
                await RPA.Logger.info('シートのリスト　　　→', TitleList);
      ...
      for (let i = 0; i <= SheetTitle3.length - 1; i++) {
        for (let n = 0; n <= TitleList.length - 1; n++) {
          ...
          if (SheetTitle3[i][0] == TitleList[n]) {
            // if (SheetTitle3[i][0] == TitleList[n]) {
            await RPA.Logger.info('シートのタイトル　　→', SheetTitle3[i][0]);
            await RPA.Logger.info('リストのタイトル　　→', TitleList[n]);
            await RPA.Logger.info(
              '視聴回数　　　　　　→',
              NumberofViewsList[n]
            );
