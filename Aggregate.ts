...
async function Start() {
  if (ErrorText.length == 0) {
    ...
    await mySSID2.push(LastMonthSheetID[0][0]);
    await mySSID2.push(ThisMonthSheetID[0][0]);
    ...
    for (let j in mySSID2) {
      for (let n in SSName4) {
        ...
        await RPA.Logger.info('本日の日付　　　　　→ ' + NewToday);
        // 前日の日付をフォーマット変更して取得
        // var yesterday = await new Date();
        // await yesterday.setDate(yesterday.getDate() - 1);
        // var year = await yesterday.getFullYear();
        // var month = (await yesterday.getMonth()) + 1;
        // var day = await yesterday.getDate();
        // var month2 = await ('00' + month).slice(-2);
        // var day2 = await ('00' + day).slice(-2);
...
async function Work(
  ...
  try {
    ...
    // 番組名からタイトルを検索
    await GetData(TitleList, NewToday, Yesterday);
    // シートに取得したデータを記載
...
async function GetData(TitleList, NewToday, Yesterday) {
  // タイトルがない場合はスキップ
  try {
    ...
    if (TitleList.length < 1) {
      ...
      if (Total[0] < 30) {
        ...
        await RPA.Logger.info('検索にヒットした動画が 30 件以上です');
        await GetData2(TitleList, NewToday, Yesterday);
      }
    }
  }
}
...
let LoopFlag = 'true';
async function GetData2(TitleList, NewToday, Yesterday) {
  // 先にアップデートされているタイトルがあるか確認
  if (MonthFlag == '今月') {
    await RPA.Logger.info('タイトルのアップロード有無を確認します');
    while (0 == 0) {
      for (let i = 0; i <= 29; i++) {
        // 一覧が出るまで待機
        await RPA.WebBrowser.wait(
      ...
      if (LoopFlag == 'false') {
        await RPA.Logger.info('前日の日付ですので取得を終了します');
        break;
      }
    }
    if (UpdateTitleList.length > 0) {
      ...
      await RPA.Logger.info('視聴回数リスト数　　→', NumberofViewsList2.length);
    } else {
      await RPA.Logger.info('本日のアップロードはありませんでした');
    }
    if (UpdateTitleList.length > 30) {
      await RPA.Logger.info('再度タイトルを入力します');
      const DeliteIcon = await RPA.WebBrowser.findElementById('delete-icon');
      await RPA.WebBrowser.mouseClick(DeliteIcon);
      await RPA.sleep(1000);
      await SetProgram(ProgramName);
      await GetData3(TitleList);
    } else {
      await RPA.Logger.info('そのまま視聴回数の取得を開始します');
      await GetData3(TitleList);
    }
  } else {
    await RPA.Logger.info('アップロード分の取得をスキップします');
    await GetData3(TitleList);
  }
}
...
// let LoopFlag3 = 'true';
let FullCount = 'false';
async function GetData3(TitleList) {
  let MatchCount = [];
  let NotMatchCount = [];
  let NotMatchCount2 = [];
  let NotMatchCount3 = [];
  while (0 == 0) {
    ...
    for (let i in TitleList) {
      for (let n in YoutubeTitleList) {
        if (YoutubeTitleList[n].indexOf(TitleList[i]) == 0) {
          // if (TitleList.length == NumberofViewsList.length) {
          // if (TitleList[TitleList.length] != YoutubeTitleList[n]) {
          //   await RPA.Logger.info('最後のタイトルが一致しました');
          //   LoopFlag2 = 'false';
          //   break;
          // } else {
          await RPA.Logger.info('シートのタイトル　　→', TitleList[i]);
          ...
          await RPA.Logger.info('視聴回数　　        →', NumberofViews);
          await NumberofViewsList.push(NumberofViews);
          MatchCount[0] = TitleList.length - Number(i) - 1;
          await RPA.Logger.info('現在のカウント　　　→', MatchCount[0]);
          // ここで取得のループを抜ける
          if (TitleList[TitleList.length - 1] == YoutubeTitleList[n]) {
            await RPA.Logger.info('最後のタイトルが一致しました');
            LoopFlag2 = 'false';
            break;
          } else {
            NotMatchCount[0] = Number(n) + 1;
            await RPA.Logger.info('現在の無カウント　　→', NotMatchCount[0]);
          }
          break;
          // }
        }
        ...
        // ここが２回取得している根源？
          ...
          NotMatchCount2[0] = Number(n) + 1;
          await RPA.Logger.info('現在の無カウント②　→', NotMatchCount2[0]);
          if (NotMatchCount2[0] == 30) {
            await RPA.Logger.info('現在の無カウント③　→', NotMatchCount2[0]);
            await RPA.Logger.info('無カウント②が 30 ですのでストップします');
            FullCount = 'true';
            break;
          }
        }
    ...
    // if (LoopFlag2 == 'true') {
    //   // テスト①
    //   await RPA.Logger.info('現在のカウント　　　→', NotMatchCount[0]);
    //   await RPA.Logger.info('現在の無カウント　　→', NotMatchCount[0]);
    //   await RPA.Logger.info('現在の無カウント⑤　→', NotMatchCount2[0]);
    ...
    if (LoopFlag2 == 'false') {
      ...
      await RPA.Logger.info('タイトルリストをクリアします');
      YoutubeTitleList = [];
      await RPA.Logger.info('カウントをクリアします');
      MatchCount = [];
      NotMatchCount = [];
      await RPA.Logger.info('視聴回数リスト数　　→', NumberofViewsList.length);
      if (MonthFlag == '今月') {
