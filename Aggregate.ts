            ...
            for (let i = 0; i <= Total[0] - 1; i++) {
              ...
              if (split[0] == NewToday && MonthFlag == '今月') {
                ...
                const NumberofViews: WebElement = await RPA.WebBrowser.driver.executeScript(
                  ...
                  `return document.getElementsByClassName('style-scope ytcp-video-row cell-body tablecell-views sortable right-align')[${i}].outerText`
                );
                const NewNumberofViews = String(NumberofViews);
                if (TitleText.indexOf('CMS用') > -1) {
                  ...
                  await NumberofViewsList.push(NewNumberofViews);
                }
              }
            }
          } else {
            ...
            for (let n in YoutubeTitleList) {
              if (YoutubeTitleList[n].indexOf(TitleList[i]) == 0) {
                ...
                const NumberofViews: WebElement = await RPA.WebBrowser.driver.executeScript(
                  ...
                  `return document.getElementsByClassName('style-scope ytcp-video-row cell-body tablecell-views sortable right-align')[${n}].outerText`
                );
                const NewNumberofViews = String(NumberofViews);
                await RPA.Logger.info(
                  '視聴回数　　        →',
  ...
  if (TitleList.length < 1) {
    ...
    if (MonthFlag == '前月') {
      ...
      if (ProgramName[0] == 'アベプラ') {
        ...
        // 格納した視聴回数の先頭を削除
        // await TitleList.shift();
        // await NumberofViewsList.shift();
        // 再度シートのタイトルを取得
