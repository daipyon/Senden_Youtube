...
async function YoutubeLogin() {
  ...
  // );
  // if (MonthFlag == '今月' && ProgramName[0] == 'Mリーグ') {
  //   await RPA.WebBrowser.sendKeys(LoginID, [`${process.env.Youtube_Login_MLeagueID}`]);
  // } else {
  ...
  // );
  // if (MonthFlag == '今月' && ProgramName[0] == 'Mリーグ') {
  //   await RPA.WebBrowser.sendKeys(LoginID, [`${process.env.Youtube_Login_MLeaguePW}`]);
  // } else {
...
async function MLeagueGetData() {
  for (let i = 0; i <= 29; i++) {
    ...
    if (split[0].includes(ThisMonth) == true && split[1] == '公開日') {
      await RPA.Logger.info(
