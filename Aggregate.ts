        ...
        for (let v in ProgramList) {
          for (let k in ProgramName2) {
            if (ProgramName2[k][0] == ProgramList[v]) {
              ...
              if (MonthFlag == '前月') {
                ...
                if (ProgramName2[k][0] == 'アベプラ') {
                  ...
                  WorkData[0] = await RPA.Google.Spreadsheet.getValues({
                    spreadsheetId: `${SSID2[j]}`,
                    range: `${SSName4[n]}!Q${Row3[0] + 7}:Q${LastRow2}`
                  });
                } else {
                  WorkData[0] = await RPA.Google.Spreadsheet.getValues({
                    spreadsheetId: `${SSID2[j]}`,
              ...
              // タイトルリストの末尾を削除
              await TitleList.pop();
              await Work(Today, NewToday, TwoMonthsBefore);
              break;
            }
          }
        }
...
async function Work(Today, NewToday, TwoMonthsBefore) {
  try {
    // Youtube Studioにログイン
    ...
    // 番組名を入力
    await SetProgram(Today, NewToday);
    // 番組名からタイトルを検索
    await GetData(NewToday, TwoMonthsBefore);
    // シートに取得したデータを記載
    await SetData(Today, NewToday);
  } catch (error) {
    ErrorText[0] = error;
    await Start();
  }
...
async function GetData(NewToday, TwoMonthsBefore) {
  // タイトルがない場合はスキップ
  try {
...
// 番組名が【スポット】の場合の処理
async function Spot(TitleList, NumberofViewsList, Today, NewToday) {
...
const Row4 = [];
const Row5 = [];
async function SetData(Today, NewToday) {
  if (TitleList.length < 1) {
