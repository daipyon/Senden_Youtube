  ...
  while (0 == 0) {
    ...
    for (let i = 0; i <= Total[0] - 1; i++) {
      try {
        ...
        //   `/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[30]/ytcp-content-section/ytcp-video-section-content/div/ytcp-video-row[${i +
        //     1}]/div/div[3]/span/div[1]`
        // );
        ...
        if (CancelProcessText == '処理を中止しました') {
          await RPA.Logger.info(`${i + 1} 行目　取得をスキップします`);
        }
      } catch {
        // タイトルを取得
        ...
        const Status = await RPA.WebBrowser.findElementByXPath(
          ...
          // 4/30 パス変更
          `/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[30]/ytcp-content-section/ytcp-video-section/ytcp-video-section-content/div/ytcp-video-row[${i +
            1}]/div/div[3]/div/div/span`
        );
